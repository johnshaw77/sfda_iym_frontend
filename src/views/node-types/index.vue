      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="類型鍵值" prop="typeKey">
          <el-input
            ...
            placeholder="請輸入唯一的節點類型鍵值（5-15字元）"
            @input="(value) => handleTypeKeyInput(value, form)"
            minlength="5"
            maxlength="15"
          />
          <span class="text-xs text-gray-500 mt-1"
...
const nodeCategories = computed(() =>
  nodeTypesConfig.categories.map(({ id, label, icon }) => ({
    value: id,
    label,
    icon,
  }))
...
const loadNodeTypes = async () => {
  try {
    ...
    // 更新 nodeTypesConfig
    nodeTypesConfig.categories = nodeTypesConfig.categories.map((category) => ({
      ...category,
      nodes: nodeTypes
        ...
        .map((node) => ({
          ...
          name: node.name,
          icon: Component, // 暫時都使用 Component 圖標
...
const handleSubmit = async () => {
  try {
    ...
    if (isEdit.value) {
      await updateNodeType(form.value.typeKey, data);
      ElMessage.success("節點類型更新成功");
    } else {
      await createNodeType(data);
      ElMessage.success("節點類型創建成功");
    }

    dialogVisible.value = false;
    loadNodeTypes(); // 重新載入列表
  } catch (error) {
...
const handleDeleteNode = async () => {
  ...
  try {
    ...
    await deleteNodeType(activeNode.value.typeKey);
    ElMessage.success("節點類型刪除成功");
...
// 處理節點類型鍵值輸入
const handleTypeKeyInput = (value) => {
  if (!value) return;
...
const rules = {
  ...
  typeKey: [
    { required: true, message: "請輸入節點類型鍵值", trigger: "blur" },
    {
      pattern: /^[a-z0-9-]+$/,

  ...
  elements.value = [
    {
      ...
      data: {
        ...activeNode.value,
        title: "test", //activeNode.value.name,
        description: activeNode.value.description,
        icon: getNodeIcon(
          activeNode.value.category,
          activeNode.value.definitionKey
        ),
        headerBgColor: activeNode.value.headerBgColor,
        headerBorderColor: activeNode.value.headerBorderColor,
      },
...
// 修改 handleNameInput 函數
const handleNameInput = (value) => {
  if (!activeNode.value) return;
  activeNode.value.name = value;
  updatePreview();
};

// 添加 handleDescriptionInput 函數
const handleDescriptionInput = (value) => {
  if (!activeNode.value) return;
  activeNode.value.description = value;
  updatePreview();
};
</script>
